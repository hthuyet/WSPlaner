        ...
        link: function (scope, element, attrs) {
            ...
            MediaStreamTrack.getSources(function (mediaSources) {
                ...
                if (videoSources.length > 1) {
                    $scope.$emit('multipleVideoSources');
                }
...
function autoComplete($compile, $timeout) {
    ...
    return {
        ...
        replace: true,
        link: function (scope, element, attr, ctrl) {
            var html_input = '<input type="text" id="autocomplete"  ng-model-options="{ debounce: 300 }" class="form-control"  ng-model="complaint" ng-keyup="checkKeyPress($event)" ng-change="complete(complaint)">'
